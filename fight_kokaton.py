...
class Score:
    """
    ゲーム中のスコアを管理するクラス
    初期設定を行うdef __init__(self)とス　コアを更新し、画面に表示するdef update(self, screen: pg.Surface):がある.
    """
    def __init__(self):
        """
        スコアの初期設定を行う
        ...
        self.img = self.fonto.render(f"Score: {self.score}", 0 , self.color)  # スコア表示用のSurfaceを生成
    ...
    def update(self, screen: pg.Surface):
        ...
        引数 screen: 画面Surface
        """
        self.img = self.fonto.render(f"Score: {self.score}", True, self.color)  # 現在のスコアを更新
...
def main():
    ...
    while True:
        ...
        # ビームリストをNoneでない要素だけに更新　
        beams = [beam for beam in beams if beam is not None]

        # 画面外に出たビームを削除
        for beam in beams:
            if not check_bound(beam.rct)[0]:  # 横方向のチェック
                beams.remove(beam)  # 画面外に出たビームをリストから削除
            else:
                beam.update(screen)  # ビームを画面に描画
